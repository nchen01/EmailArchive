...
| S17 | Audited handoff package MVP — employee-initiated scope review, package publish, one-time recipient link, read-only recipient view, deterministic package-local ask (top-level tab), three-part recipient coverage workspace with claim-attached evidence, revoke, new-version re-share/supersede, static HTML export, empty-generation diagnostic, refresh-safe workspace + demo seed, manual-demo runbook, audit lifecycle | ✓ done (S17.2–S17.19); e2e-validated | `docs/s17-handoff-package-mvp-plan.md`, `docs/s17-live-validation.md` |
