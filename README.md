...
**Status:** S0–S16.0 complete; **S17.2–S17.20 audited handoff package MVP (D14) shipped and end-to-end validated**. The current working demo path is the shipped S17 handoff-demo seed + runbook (`scripts/seed_handoff_demo.py`, `docs/s17-handoff-manual-demo-runbook.md`): the seeded **handoff-demo** mailbox for Handoff package generation, and **puluo** for Cover-for-me / Relationship Map / Network Map. The older **S16 "canonical demo readiness"** (a broader purpose-built demo-story / landing fixture) is **superseded in practice for the handoff demo** and remains only optional future polish — not a blocker. S17.2–S17.20 deliver the employee-initiated flow: create draft → scope → generate → review/remove evidence → publish to one recipient with a one-time capability link → read-only recipient view at `/handoff/recipient` (a compact three-part coverage workspace — area rail · brief · people/ask, S17.17–S17.18, with a deterministic LLM-free package-local ask, S17.9) → revoke → create a revised version in the same lineage that supersedes the prior one (S17.10) → export a self-contained static HTML snapshot (S17.11), all audited; S17.13 adds manual-demo readiness (creator "Start over" fix + a creator-only empty-generation diagnostic explaining why a package generated no claims), and S17.14 makes the creator workspace mailbox refresh-safe (sessionStorage) and adds a deterministic, LLM-free handoff-demo seed (`scripts/seed_handoff_demo.py`) so the full flow can be exercised end to end, and S17.15 adds a canonical manual-demo runbook (`docs/s17-handoff-manual-demo-runbook.md`) plus a `--verify` seed mode; S17.16 makes the Handoff demo mailbox's Network/Relationship tabs degrade to an empty graph instead of a 404, and S17.17–S17.19 reshape the recipient view into a compact three-part coverage workspace (area rail · brief · people) with a top-level Ask tab and evidence attached inline to each claim (`docs/s17-live-validation.md`). S17.20 widens the creator Handoff review into a workspace layout with a full-width package header and a sticky action rail so Publish is reachable without scrolling through evidence (mobile promotes actions to the top), plus an Overview-readiness clarification note. Deferred beyond S17: optional LLM synthesis for the package ask, PDF/docx/zip export, manager approval, multi-recipient, rich snapshotted relationship/project/owner trees inside the package (S17.17–S17.19 ship a package-local topic workspace, not a graph), and a stronger production auth boundary. S16.0 date-range ingest is complete (custom date-window preview/ingest, scoped snapshot mode, replace-snapshot guardrails). S7 L2 retrieval live-validated (S7.1–S7.11; optional S7.12 hosted Voyage reranker remains off by default). S8 real-mailbox demo readiness complete. S9 project-clustering materialization complete (`scripts/materialize_projects.py`). S10 local runtime reliability complete (Voyage embedding via direct HTTP, runtime preflight probe, blessed Windows launch scripts, no-hang frontend). S11 demo polish complete (inspectable citation evidence drawer, deduped citations, display-only project label cleanup, readiness strip). S12 product shell + landing complete (client routing, workspace overview, marketing landing page, Cover-for-me onboarding). S13 Relationship Map complete (new graph-backed, tree-renderable relationship view derived live from L1 tables; owner/project/org/graph modes; sensitive-thread exclusion; Network Map preserved). S14 evidence/source-navigation polish complete (safe source-message detail endpoint, richer evidence drawer, Gmail best-effort search link, structural relationship provenance notes). S15 verification hardening complete (S9 DB-test contamination fixed; `docs/s15-verification-matrix.md` defines local, DB-gated, demo, and live green tiers). See `docs/decisions.md`.
**S18 (shipped as a docs-only plan; not implemented in code)** maps hosted product readiness / web-app deployment —
...
code this sprint. See `docs/s18-hosted-product-readiness-plan.md`. **S19 (shipped
as a spec; not implemented in code)** specifies the production auth + tenant boundary that S22+ will
...
**S20 (shipped as a spec; not implemented in code)** specifies safe Gmail OAuth connect + a token
...
`docs/s20-oauth-token-vault-plan.md`. **S21 (shipped as a spec; not implemented in code)** specifies
...
only. No migration (head stays `0012_job_infra`); recipient routes
untouched. **S30 adds the two audited admin mutations** — `POST /api/admin/packages/{id}/revoke`
and `POST /api/admin/provider-accounts/{id}/disconnect`, both tenant-admin-only with
a mandatory reason (422 if blank), cross-tenant → 404. Revoke reuses the S17 creator
revoke lifecycle (blocks recipient access + kills live sessions) and audits
...
| Queue / worker | Postgres-backed `job` table + worker (`SELECT … FOR UPDATE SKIP LOCKED`) — S21 recommended default | Long/retryable ingest/enrichment jobs run outside web requests. Synchronous for local/fixture runs; an external broker (Redis/SQS/etc.) is **deferred** until throughput demands it (S21 §11). |
...
| S16 | Canonical demo readiness — broader purpose-built demo-story / landing fixture. **Superseded in practice** by the shipped S17 handoff-demo seed + runbook (handoff-demo mailbox + puluo) for the current demo path; remains optional future polish | superseded / optional | `docs/s16-demo-readiness-plan.md` |
| S17 | Audited handoff package MVP — employee-initiated scope review, package publish, one-time recipient link, read-only recipient view, deterministic package-local ask (top-level tab), three-part recipient coverage workspace with claim-attached evidence, revoke, new-version re-share/supersede, static HTML export, empty-generation diagnostic, refresh-safe workspace + demo seed, manual-demo runbook, audit lifecycle | ✓ done (S17.2–S17.20); e2e-validated | `docs/s17-handoff-package-mvp-plan.md`, `docs/s17-live-validation.md` |
...
# 4. Tests
DATABASE_URL=postgresql+psycopg2://ekc:ekc_dev_password@localhost:5432/ekc_dev \
  pytest                                # see docs/s15-verification-matrix.md for the correct green tier; full DB green uses ekc_test
